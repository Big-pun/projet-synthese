    ...
    <form @submit.prevent="handleSubmit" class="space-y-4 p-4">
      ...
      <div class="mb-4">
        ...
        <div class="relative h-20">
          <button @click="toggleAllPasswordVisibility" class="absolute right-5 top-1/2 -translate-y-1/2 mb-2 rounded-md bg-dark-gray border-2 border-gray p-2">
            <p class="text-gray">mots de passe</p>
          </button> 
        </div>

        <div class="rounded-lg bg-white p-4 sm:flex sm:flex-row items-center">
          <label for="currentPassword" class="block text-sm font-medium text-gray responsive-margin">
            Mot de passe actuel
          </label>
        ...
        <div v-if="v$.currentPassword.$error" class="text-accent2 text-sm mt-1 ml-4">
      ...
      <div class="mb-4">
        <div class="rounded-lg bg-white p-4 sm:flex sm:flex-row items-center">
          <label for="newPassword" class="block text-sm font-medium text-gray responsive-margin">
            Nouveau mot de passe
          </label>
          <div class="relative w-full">
            <input 
        ...
        <div v-if="v$.newPassword.$error" class="text-accent2 text-sm mt-1 ml-4">
      ...
      <div class="mb-4">
        <div class="rounded-lg bg-white p-4 sm:flex sm:flex-row items-center">
          <label for="confirmPassword" class="block text-sm font-medium text-gray responsive-margin">
            Confirmer le mot de passe
          </label>
          <div class="relative w-full">
            <input 
              ...
              @input="checkPasswordsMatch"
              class="w-full px-3 py-2 border rounded-md focus:border-accent1 outline-none"
              :class="{ 'border-accent2': v$.confirmPassword.$error || !passwordsMatch }"
            />
            <button 
              type="button" 
        ...
        <div v-if="v$.confirmPassword.$error" class="text-accent2 text-sm mt-1 ml-4">
          {{ v$.confirmPassword.$errors[0].$message }}
        </div>
        <div v-if="!v$.confirmPassword.$error && !passwordsMatch" class="text-accent2 text-sm mt-1 ml-4">
...
function toggleAllPasswordVisibility() {
  ...
  const atLeastOneVisible = Object.values(passwordVisibility).some(value => value === true);
