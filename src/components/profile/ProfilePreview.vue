...
<template>
  <div class="mb-8 font-roboto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 space-y-6 lg:space-y-0">
      <!-- preview card -->
      <div class="self-center bg-light-gray text-lg shadow-sm rounded-3xl border-b-8 flex flex-col items-center max-w-[500px] mx-auto w-full py-4 border-accent2">
        <h2 class="text-center text-accent2 mb-4 font-medium font-roboto">Aperçu</h2>
        <div class="flex flex-col md:flex-row items-center space-x-4 w-full justify-center lg:justify-start">
          ...
          <div class="flex flex-col text-gray space-y-1 font-medium text-center md:text-left">
            <p class="text-xs sm:text-base">{{ userInfo.firstName }} {{ userInfo.lastName }}</p>
            <p class="text-xs sm:text-base">{{ userInfo.email }}</p>
            <p class="text-xs sm:text-base">{{ userInfo.phone }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="flex text-lg flex-col justify-center items-center lg:items-end space-y-4 md:space-y-6">
        <button
          ...
          class="text-sm md:text-base w-full max-w-xs md:max-w-sm flex items-center justify-around bg-gray text-white py-3 px-4 rounded-md hover:bg-hover transition-colors">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-9 w-8" viewBox="0 0 39 39" fill="none">
        ...
        <button
          ...
          class="text-sm md:text-base w-full max-w-xs md:max-w-sm flex items-center justify-around bg-gray text-white py-3 px-4 rounded-md hover:bg-accent2 transition-colors">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-9 w-8" viewBox="0 0 36 36" fill="none">
